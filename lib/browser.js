(function() {
        ...
        socket.emit('listen', null, function(options) {
            socket.on('synced', function() {
                ...
                _.each(backends, function(backend) {
                    if (_.isFunction(backend.onsync)) backend.onsync.apply(null, args);
                });
            });
    ...
    function buildBackend(collection) {
        ...
        socket_ready.then(function(socket, options) {
            ...
            backend.onsync = function(method, resp, name) {
                var event = options.event;

                if (collection.name === name) { // special to the BTI framework
